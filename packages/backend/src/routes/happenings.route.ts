...
// There're so fucking many dublications... When will anybody fix it wipepeepoSad
...
const update = multer({
    ...
    fileFilter: async (req: Request<{happeningId: string}>, file, cb) => {
        const {place, mapName, teamSize, eventStartDate, eventStartTime, eventEndDate, eventEndTime, description} = req.body
        ...
        const sameTime = new Date(`${eventStartDate} ${eventStartTime}`).getTime() === new Date(event.rows[0].start_at).getTime()
...
Router.put("/happenings/:happeningId/update/run", authMiddleware, HappeningsController.updateRun)
Router.put("/happenings/:happeningId/update/event", authMiddleware, update.single("thumbnail"), HappeningsController.updateEvent)
